...
def generate_databases(N_obs,true_lens_frac,cosmo_type,Om_fid,Ode_fid,H0_fid,w_fid,wa_fid,
                       verbose=False,Percentage_error=np.nan,absolute_error = np.nan):
...
#    db_filename = f'./databases/spectroscopic_db_{Percentage_error}perc_{N_obs}_samples_'+\
    ...
    db_filename = f'./databases/Gaussian_noise_{absolute_error}abs_{N_obs}_samples_'+\
