# -*- coding: utf-8 -*-
"""
Created on Thu Apr 20 22:29:02 2017
...
zt,z2,mu_obs0 = np.loadtxt('fakedata_spectroscopic.txt',usecols=[1,2,4],unpack=True)
...
for i in range(0,n-1):
    ...
    if i == 0:
        ...
        chi2_current2 = ((mu_obs0-mu_theory12)/sig_tau1)**2
        mu_theory13 = mu_interp(z_obs0,om_current,H0_current,w_current)
        chi2_current3 = ((mu_obs0-mu_theory13-offset)/sig_tau2)**2
        mu_theory14 = mu_interp(z_obs3,om_current,H0_current,w_current)
        chi2_current4 = ((mu_obs0-mu_theory14-offset)/sig_tau2)**2
        
        L1_current = P_tau*P_gamma*((1/np.sqrt(2*np.pi*sig_tau1**2))*np.exp(-chi2_current1/2))
        L2_current = P_tau*(1-P_gamma)*((1/np.sqrt(2*np.pi*sig_tau1**2))*np.exp(-chi2_current2/2))
        L3_current = (1-P_tau)*(P_gamma)*((1/np.sqrt(2*np.pi*sig_tau2**2))*np.exp(-chi2_current3/2))
        L4_current = (1-P_tau)*(1-P_gamma)*((1/np.sqrt(2*np.pi*sig_tau2**2))*np.exp(-chi2_current4/2))
        likelihood_current = L1_current + L2_current + L3_current + L4_current
    ...
    chi2_proposed2 = ((mu_obs0-mu_theory22)/sig_tau1)**2
    mu_theory23 = mu_interp(z_obs0,om_proposed,H0_proposed,w_proposed)
    chi2_proposed3 = ((mu_obs0-mu_theory23-offset)/sig_tau2)**2
    mu_theory24 = mu_interp(z_obs3,om_proposed,H0_proposed,w_proposed)
    chi2_proposed4 = ((mu_obs0-mu_theory24-offset)/sig_tau2)**2
    
    L1_proposed = P_tau*P_gamma*((1/np.sqrt(2*np.pi*sig_tau1**2))*np.exp(-chi2_proposed1/2))
    L2_proposed = P_tau*(1-P_gamma)*((1/np.sqrt(2*np.pi*sig_tau1**2))*np.exp(-chi2_proposed2/2))
    L3_proposed = (1-P_tau)*(P_gamma)*((1/np.sqrt(2*np.pi*sig_tau2**2))*np.exp(-chi2_proposed3/2))
    L4_proposed = (1-P_tau)*(1-P_gamma)*((1/np.sqrt(2*np.pi*sig_tau2**2))*np.exp(-chi2_proposed4/2))
    likelihood_proposed = L1_proposed + L2_proposed + L3_proposed + L4_proposed
