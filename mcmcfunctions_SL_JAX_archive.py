...
#         L = numpyro.factor("Likelihood",prob_1)
        ...
        if batch_bool:
            with numpyro.plate("N", zL_obs.shape[0], subsample_size = subsample_size):
                ...
                #                 a=batch_prob_1,b=batch_prob_2,c=batch_prob_3,d=batch_prob_4,e=batch_prob_5,f=batch,g=jnp.sum(batch))
                # jax.debug.print('Sum: {a},{b},{c},{d},{e}',a=jnp.sum(batch_prob_1),b=jnp.sum(batch_prob_2),c=jnp.sum(batch_prob_3),d=jnp.sum(batch_prob_4),e=jnp.sum(batch_prob_5))
