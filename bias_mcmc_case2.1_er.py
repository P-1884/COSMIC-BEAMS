...
erm = sys.argv[2] #'a' or 'b' or 'c' (how we deal with error)
...
fout = open('../data/bias_chain_fd'+ds+'_case2.1_'+erm+'_er.txt','w') 
...
for i in range(0,n-1):
    ...
    if erm == 'c':
        ...
        new_err = np.sqrt(sig_mu_obs2**2 + (dmu*shift*(1+z_obs0))**2)
        chi2_current = ((mu_obs-mu_theory1)/new_err)**2
        log_like_current = sum(np.log(1/(np.sqrt(2*np.pi*new_err**2))))-0.5*sum(chi2_current)
    else: 
        chi2_current = ((mu_obs-mu_theory1)/sig_mu_obs2)**2
        log_like_current = sum(np.log(1/(np.sqrt(2*np.pi*sig_mu_obs2**2))))-0.5*sum(chi2_current)
    """
    likelihood_current = PI((1/np.sqrt(2*np.pi*sig_mu_obs2**2))*np.exp(-chi2_current/2))
    prior = 1
    evidence = 1
    ...
    if erm == 'c':
        ...
        chi2_proposed = ((mu_obs-mu_theory2)/sig_mu_obs2)**2
        log_like_proposed = sum(np.log(1/(np.sqrt(2*np.pi*sig_mu_obs2**2))))-0.5*sum(chi2_proposed)
    """
    likelihood_proposed = PI((1/np.sqrt(2*np.pi*sig_mu_obs2**2))*np.exp(-chi2_proposed/2))
    prior = 1
    evidence = 1
